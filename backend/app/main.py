...
# VSCode 直接运行 backend/app/main.py 时，Python 只会把 backend/app 放进
# sys.path，导致 `from app...` 找不到顶层包。这里显式补上 backend/。
...
app = FastAPI(title="VeriDeep API", version="2.0.0")
...
def _run_dev_server() -> None:
    """Allow running this file directly from VSCode."""
    import uvicorn

    port = int(os.environ.get("VERIDEEP_DEV_PORT") or settings.app_port)
    uvicorn.run(
        app,
