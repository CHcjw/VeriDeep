...
export default function VSidebar() {
  ...
  return (
    <aside className="relative flex h-full w-[220px] shrink-0 flex-col border-r border-line bg-card/70">
      ...
      >
        <span className="grid h-9 w-9 place-items-center rounded-btn bg-[#062631] text-primary shadow-[inset_0_0_0_1px_rgba(53,242,208,0.16)]">
      ...
      <nav className="flex flex-1 flex-col gap-1 px-3">
        {navItems.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              [
                'flex h-11 items-center gap-3 rounded-btn px-3.5 text-[15px] transition-all ease-verideep',
                isActive
      ...
      <div className="mx-3 mb-3 rounded-card border border-line/70 bg-primary-tint/40 p-4">
        <div className="flex items-center gap-2">
          <VeriDeepMark size={16} className="text-primary" />
      ...
      <div className="flex items-center gap-3 border-t border-line px-4 py-3.5">
        ...
        <div className="min-w-0 flex-1">
          <div className="truncate text-aux font-medium text-ink">陈研究员</div>
          <div className="truncate text-tag text-ink-3">VeriDeep Lab</div>
        </div>
