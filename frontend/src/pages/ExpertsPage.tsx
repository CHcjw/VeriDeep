...
const LEVEL_BG: Record<ExpertLevel, string> = {
  L1: 'bg-primary-tint text-primary-deep',
  L2: 'bg-paper text-info',
  L3: 'bg-sun-soft text-warn',
}
...
function ExpertCard({ expert, onClick }: { expert: Expert; onClick: () => void }) {
  return (
    ...
    >
      <span className={`absolute right-2.5 top-2.5 inline-flex items-center gap-1 rounded-chip px-2 h-5 text-tag font-medium ${LEVEL_BG[expert.level]}`}>
        {expert.level}
      </span>
          ...
          {LEVEL_TABS.map((t) => (
            <button
              ...
              className={`flex flex-col items-start rounded-btn px-3.5 py-1.5 text-left transition-all ${
                tab === t.key ? 'bg-primary text-bg shadow-card' : 'bg-card text-ink-2 hover:bg-primary-tint'
              }`}
            >
